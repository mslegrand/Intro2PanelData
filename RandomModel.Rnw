...
We begin by creating some data:
<<RandomDataGenerate>>=
...
write.csv(df,"./Data/random1.csv", row.names=FALSE)
df<-NULL
@
...
$Pr(>|t|)$ exceeding $0.05$ means that z should be eliminated as an explanatory variable.
