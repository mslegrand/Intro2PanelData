...
Sometimes it is useful to generate some artificial panel data, to test our analysis algorithms. We can easily generate panel data in R using the built random number generators and a wonderful little function called {\it expand.grid}.
<<panelGenerate1>>=
