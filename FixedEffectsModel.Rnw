...
\chapter{Fixed Effects Model} \label{Chapt:Fixed.Effects.Model}
...
In practice, the dummary variables need not be insert, since we can use the country names as factors. We illustrate this in the following


we use the previous panel data, {\it panel}. This process for the panel data {\it panel} is implemented as follows:
<<fe-fit1>>=
...
p<- p + geom_line(aes(x=x1,y=yhat,group=country,colour=country)) 
p<- p + ggtitle("Fit using Dummy Variables")
p
@
...
library(plm)
fixed.within<-plm(y~x1, data=panelData, index=c("country","year"), model="within")
...
<<>>=
yhat<-hat(panelData$x1,panelData$country)
library(ggplot2)
panelData$yhat<-yhat
p<-ggplot(panelData)
p<- p + geom_point(aes(x=x1,y=y,group=country,colour=country) ) 
...
p<-ggplot(panelData)
p<- p + geom_point(aes(x=x1,y=y,group=country,colour=country) ) 
p<- p + geom_abline(intercept=pooled$coefficients[1], slope=pooled$coefficients[2], colour="red", size=1) 
p<- p + geom_line(aes(x=x1,y=yhat,group=country,colour=country)) 
p<- p + ggtitle("Fixed Using the Within Estimator")
p
@
...
panelData<-read.dta("http://dss.princeton.edu/training/Panel101.dta") 
library(plm)
model.fixed<-plm(y~x1, data=panelData, index=c("country","year"), model="within")
model.pooled<-plm(y~x1, data=panelData, index=c("country","year"), model="pooling")
...
T<-c(2001,2002,2003,2004,2005)
panel<-expand.grid(country=country, year=T)
panel$x<-runif(nrow(panel), 10, 20)
library(plyr)
delta<-daply(panel, .(country), function(df){sum(df$x)})
delta<-delta/sqrt(delta%*%delta)
alpha<-5
epsilon<-runif(nrow(panel),-1,1)
panel$y<-alpha + delta + 2*df$x + epsilon
@
